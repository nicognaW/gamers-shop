...
fun findAndAddClassesInPackageByFile(
    packageName: String, packagePath: String, recursive: Boolean, classes: MutableSet<Class<*>>
) {
    // 获取此包的目录 建立一个File
    ...
    for (file: File in dirfiles) {
        ...
        if (file.isDirectory) {
            ...
            val className = file.name.substring(
...
fun getClasses(pack: String): Set<Class<*>> {
    val logger = LoggerFactory.getLogger("ClassScanner")
    ...
    try {
        dirs = Thread.currentThread().contextClassLoader.getResources(
            packageDirName
        )
        // 循环迭代下去
        while (dirs.hasMoreElements()) {
            ...
            if ("file" == protocol) {
                ...
                findAndAddClassesInPackageByFile(
                    packageName, filePath, recursive, classes
                )
